...
\begin{abstract}
\noindent Urban last-mile logistics---same-day delivery, ride-hailing, crowdsourced pickup---requires that the cost of a single shared tour be divided among customers whose identities are revealed only as the vehicle dispatches. Static cooperative routing models silently assume every customer is available from the outset, distorting fairness when arrivals are spread in time. We introduce the Online Traveling Salesman Game (Online TSG), in which the family of feasible coalitions is endogenously induced by the realized arrival-service process; the classical static game is recovered at simultaneous arrivals. Our main theoretical result identifies three complementary sufficient conditions under which the Core of this restricted game empties---a single-complement threshold tied to the minimum residual customer cost, a balanced-complement threshold of Bondareva--Shapley type, and a balanced-near-complement threshold on size-(n-1) and (n-2) coalitions---and proves that all three are structurally blocked whenever the peak waiting queue satisfies $k<n-1$ (intermediate-coalition emptiness can persist in this regime; Observation~\ref{obs:intermediate}). Cooperative cost allocations on this restricted family are computed via a sequential LP cascade (the Temporal Nucleolus). The single-complement threshold tightens at the Beardwood--Halton--Hammersley $\sqrt{n}$ rate as $n$ grows. On 175 nearest-neighbor instances 67 Cores are empty; three complement-family thresholds certify 57 and a partition-pair certificate the remaining 9 (66 of 67, no false positives). Extending to cheapest-insertion and batch re-optimization (525 policy-instance pairs) yields no false positives and eliminates an intermediate-coalition mechanism observed only under nearest-neighbor. Scale-up to 50 customers confirms the asymptotic tightening. In our synthetic single-vehicle experiments, short service-time promises (peak queue $k<n-1$) structurally block all complement-coalition mechanisms, and combined with cheapest-insertion or batch re-optimization dispatch they substantially improve empirical Core stability.
\end{abstract}
...
\item[\textbf{C4.}] \emph{Empirical calibration.} On 175 simulated instances across five coalition sizes and seven arrival patterns under three dispatch policies (525 policy-instance pairs), plus 45 scale-up instances up to $n=50$ and a 24-instance scale-invariance study (Supplementary Materials, \S S2), we verify every theoretical claim. Under NN dispatch, Theorem~\ref{thm:empty-core} fires in 37 instances (all observed empty), Proposition~\ref{prop:bondareva-complement} adds 11 balanced-complement certifications, and Proposition~\ref{prop:balanced-near-complement} adds 9 balanced-near-complement certifications (jointly 57 of 67 observed empties); the remaining 10 split as 1 near-complement residual (threshold-bound by $\approx 2\times 10^{-3}$) and 9 intermediate-coalition cases certified by Proposition~\ref{prop:partition-pair} (66 of 67 total). No false positives arise under any policy. The mean thresholds ($\bar r^{\ast\ast}=1.223$, $\bar r^{\ast\ast\ast}=1.096$) are sharper than the classical bound ($\bar r^{\ast}=4.351$) by factors of $3.56$ and $3.97$. Corollary~\ref{cor:cor52} holds analytically without exception: no instance with $k<n-1$ satisfies any of the three thresholds; the 9 intermediate-coalition empties all lie at $k<n-1$ under NN and vanish under cheapest-insertion and batch-reoptimization. The asymptotic decay $r^{\ast\ast}\to 1$ is consistent with Theorem~\ref{thm:asymptotic}'s $O(n^{-1/2})$ rate on the scale-up grid.
\end{itemize}

Operationally these results say: a bounded peak queue $k<n-1$---achievable via short service-time promises---eliminates all complement-coalition mechanisms (Theorem~\ref{thm:empty-core}, Propositions~\ref{prop:bondareva-complement} and~\ref{prop:balanced-near-complement}), covering the pathological ``morning batch'' regime. In the shallow-queue regime the intermediate-coalition mechanism of Observation~\ref{obs:intermediate} remains active but is policy-sensitive: observed only under nearest-neighbor dispatch and absent under cheapest-insertion or batch re-optimization (Section~\ref{ssec:policy}). Short service windows combined with moderately informed dispatch substantially improve empirical Core stability, though intermediate-coalition emptiness remains logically possible at high competitive ratios.

\paragraph{Structure.} The Online TSG is a single-vehicle cooperative game; this scope isolates the temporal coalition-feasibility structure of $\F$ from fleet-assignment confounds and is discussed further in Section~\ref{sec:conclusion} (where a multi-vehicle Online VRG is deferred to a companion manuscript). Section~\ref{sec:related} situates our work within the literature on static TSG/VRG, dynamic and rolling-horizon games, online cooperative games, and nucleolus computation. Section~\ref{sec:problem} formalizes the Online TSG. Section~\ref{sec:nucleolus} defines and computes the Temporal Nucleolus. Section~\ref{sec:theory} develops the Core theory. Section~\ref{sec:experiments} reports computational experiments, and Section~\ref{sec:conclusion} concludes.
...
$r^{\ast\ast\ast}$ & $1+\bigl(\sum_{i\in N}\delta_i\bigr)/\bigl((n-1)\,\cstar(N)\bigr)$: balanced-complement threshold (Prop.~\ref{prop:bondareva-complement}); requires $B_{n-1}\subseteq\F$ \\
$r^{(\diamond)}_{\min}$ & LP optimum of Bondareva--Shapley over $B_{n-1}\cup B_{n-2}$ (Prop.~\ref{prop:balanced-near-complement}); requires $B_{n-1}\cup B_{n-2}\subseteq\F$ \\
...
\paragraph{Algorithmic scope.} All Core-stability results of this paper depend only on the first-stage LP $\varepsilon_1^{\ast}$; tie-breaking and primal-degeneracy refinements are needed only for the full Nucleolus point and are described in Supplementary~\S\ref{app:algorithmic-details}.
...
\label{cor:cor51}
If arrivals are fully sequential in the sense of Supplementary Lemma~\ref{lem:lemma52}, then Theorem~\ref{thm:empty-core} is vacuous: no $N\setminus\{i\}$ is feasible, so its hypothesis cannot hold.
\end{corollary}
...
Beyond Theorem~\ref{thm:empty-core} and Proposition~\ref{prop:bondareva-complement}, we observe main-grid instances (10 of 175 under the NN policy) where the Temporal Core is empty, both thresholds are vacuous, and the binding constraints in the Core LP mix size-$(n-1)$ and size-$(n-2)$ coalitions simultaneously. Proposition~\ref{prop:balanced-near-complement} below formalises this mechanism as a Bondareva--Shapley sufficient condition on the mixed collection $B_{n-1}\cup B_{n-2}$; it fires on 9 of the 10 observed cases (Section~\ref{ssec:cor16}, Table~\ref{tab:thm5}; Supplementary Materials \S S4).
\end{remark}

\begin{proposition}[Balanced-near-complement threshold]
\label{prop:balanced-near-complement}
...
Setting $\lambda_S=1/(n-1)$ on $B_{n-1}$ and $\lambda_S=0$ on $B_{n-2}$ recovers Proposition~\ref{prop:bondareva-complement} on the subdomain where Proposition~\ref{prop:balanced-near-complement}'s feasibility hypothesis $\F\supseteq B_{n-1}\cup B_{n-2}$ holds; on this subdomain Proposition~\ref{prop:balanced-near-complement} is a refinement, optimizing $\lambda$ over the larger collection. Note that the two propositions have different applicability domains: Proposition~\ref{prop:bondareva-complement} requires only $\F\supseteq B_{n-1}$, while Proposition~\ref{prop:balanced-near-complement} additionally requires $\F\supseteq B_{n-2}$. The sharpest threshold $r^{(\diamond)}_{\min}$ is the optimum of a linear program with $n+\binom{n}{2}$ variables and $n$ equality constraints; Section~\ref{ssec:cor16} reports the empirical fire-count (9 of 10).

\begin{proposition}[General partition-pair threshold]
\label{prop:partition-pair}
...
\paragraph{Relationship to earlier thresholds.} Theorem~\ref{thm:empty-core} is the $|S|=1$ specialization of Proposition~\ref{prop:partition-pair}: with $S=\{i\}$, $r^{(P)}_{\{i\}} = 1+\delta_i/\cstar(N)$ recovers $r^{\ast\ast}$, and the feasibility hypothesis $S,N\setminus S\in\F$ reduces to $N\setminus\{i\}\in\F$. The novelty of Proposition~\ref{prop:partition-pair} is that $r^{(P)}_{\min}$ remains \emph{defined} when no \emph{singleton} complement is feasible, provided some \emph{non-singleton} partition pair has both halves in $\F$. This is precisely the regime $k<n-1$: although every $N\setminus\{i\}$ is infeasible (Corollary~\ref{cor:cor52}), a partition $(S,N\setminus S)$ with $|S|,|N\setminus S|\in\{2,\ldots,n-2\}$ can still have both halves admissible. Proposition~\ref{prop:partition-pair} is therefore the first Bondareva--Shapley threshold surviving the structural obstruction of Corollary~\ref{cor:cor52}; the empirical content of Observation~\ref{obs:intermediate} below is exactly that this regime is non-empty. Operationally, Proposition~\ref{prop:partition-pair} is most naturally read as a \emph{certificate} rather than a predictive bound: given a feasible partition pair, it provides analytic verification of Core emptiness, but locating the partition pair $(S,N\setminus S)$ that minimizes $r^{(P)}_S$ in general requires either an enumeration over feasible partition pairs of $\F$ or extraction from the dual support of the first-stage Core LP (see Supplementary~\S\ref{app:partition-pair-cases}).

\begin{observation}[Intermediate-coalition mechanism, certified by Proposition~\ref{prop:partition-pair}]
\label{obs:intermediate}
In 9 of the 175 main-grid instances under the NN policy, the Temporal Core is empty despite (i) the hypothesis of Theorem~\ref{thm:empty-core} being vacuous ($r\le r^{\ast\ast}$, or no singleton complement feasible) and (ii) the hypothesis of Proposition~\ref{prop:bondareva-complement} being vacuous. All 9 satisfy $k<n-1$ and arise in patterns $\text{B}_{\text{medium}}$ (7 cases) and E (2 cases) with realized competitive ratio $r\in[1.33,1.61]$. The binding constraints in the Core LP concentrate on intermediate-size coalitions ($2\le |S|\le n-3$) rather than on complements or near-complements. The first-stage Core-LP dual of each of the 9 cases is supported on exactly one partition pair $(S,N\setminus S)$ with $|S|,|N\setminus S|\in\{2,\ldots,n-2\}$; the corresponding partition-pair threshold $r^{(P)}_{S}$ falls strictly below the realized $r$ in every case (margins $0.008$--$0.152$), so Proposition~\ref{prop:partition-pair} certifies all 9 cases analytically. Detailed per-case partition pairs appear in Supplementary~\S\ref{app:partition-pair-cases}.
\end{observation}
...
A trivial inheritance result holds in the special case of exact online dispatch: when $C(N)_{\text{online}}=\cstar(N)$ every static-TSG Core allocation remains in the Temporal Core, since efficiency matches and every $\Fp$-constraint is already implied by the static Core (Supplementary Proposition~\ref{prop:core-inheritance}, \S\ref{app:inheritance}). Operationally this is informative only in the simultaneous-arrival regime where it coincides with the static Core itself; the structurally informative survival statement is the obstruction at $k<n-1$.

\begin{corollary}[Structural obstruction of the complement mechanisms]
\label{cor:cor52}
If the peak waiting queue satisfies $k=\max_t |U_t| < n-1$, then no coalition of the form $N\setminus\{i\}$ is feasible, so Theorem~\ref{thm:empty-core}, Proposition~\ref{prop:bondareva-complement}, and Proposition~\ref{prop:balanced-near-complement} are each vacuous: none of their feasibility hypotheses can hold. The only remaining \emph{a priori} mechanism for emptying the Temporal Core under $k<n-1$ is Remark~\ref{rem:remark1}, which is dominated by $r^{\ast}$ and rarely binds in practice. Intermediate-coalition violations remain logically possible; their empirical frequency is characterized in Section~\ref{sec:experiments}.
\end{corollary}

\begin{proof}
If $N\setminus\{i\}\in\F$ for some $i$, then $|U_t|\ge n-1$ at some $t$, contradicting $k<n-1$. Theorem~\ref{thm:empty-core}'s hypothesis requires one feasible complement, and Propositions~\ref{prop:bondareva-complement} and~\ref{prop:balanced-near-complement} require $B_{n-1}\subseteq\F$ (strictly stronger); each is therefore violated. The final claim follows from Theorem~\ref{thm:empty-core} and Remark~\ref{rem:remark1}.
\end{proof}
...
\item Arrival patterns interpolate between two extremes: simultaneous ($k=n$, $\F=2^N\setminus\{\emptyset\}$, classical TSG) and fully sequential ($k\le 1$ for sequential arrival regimes, Theorem~\ref{thm:empty-core} vacuous).
\end{itemize}
In queueing-theoretic terms, $k\ge n-1$ with positive probability activates the complement-coalition mechanism; otherwise Corollary~\ref{cor:cor52} blocks it, though intermediate-coalition and near-complement violations (Remark~\ref{rem:near-complement}) remain logically possible.
...
\end{itemize}
The seven patterns form a $(\rho,\text{structure})$ factorial rather than a $\rho$-only sweep: the load axis is traced by $A$ and the three uniform-sequential B-rates ($\rho\in\{0.5,1,2,4,\infty\}$); at the saturation-borderline $\rho=2$ a uniform-sequential pattern ($\text{B}_{\text{medium}}$) and a clustered pattern ($\text{C}$) are paired, and at $\rho=1$ a reverse-angle pattern ($\text{D}$) and a random pattern ($\text{E}$) are paired, isolating arrival geometry from arrival rate at fixed $\rho$. Patterns $\text{B}_{\text{medium}}$ and $\text{C}$ share $\rho=2$ but differ in arrival structure; Section~\ref{ssec:pattern} shows that this structural difference alone suffices to place them in qualitatively distinct regimes of Core stability, illustrating that the load parameter $\rho$ is necessary but not sufficient to characterize dynamics.
...
\begin{table}[t]
\centering
\caption{Five-mechanism contingency on the 175-instance NN grid. The three complement-based analytic thresholds (Theorem~\ref{thm:empty-core}, Proposition~\ref{prop:bondareva-complement}, Proposition~\ref{prop:balanced-near-complement}) jointly certify 57 of the 67 observed empty Cores, and the general partition-pair threshold (Proposition~\ref{prop:partition-pair}) certifies the remaining 9 intermediate-coalition cases of Observation~\ref{obs:intermediate} that operate at $k<n-1$ (66 of 67 jointly). One near-complement case eludes all four (Remark~\ref{rem:near-complement} residual). No false positives arise under any policy. Row labels denote the best-discriminating mechanism per instance, not the only threshold that fires. Standalone fire counts overlap: under NN dispatch, Proposition~\ref{prop:bondareva-complement}'s bound $r>r^{\ast\ast\ast}$ is satisfied in 57 of the 175 instances (all 37 Theorem~\ref{thm:empty-core} fires plus 20 further cases, of which 11 lack any size-$(n-2)$ binding and are labeled ``balanced-complement'' (best label) and 9 also exhibit size-$(n-2)$ binding and are labeled ``balanced-near-complement'' (best label)). Proposition~\ref{prop:balanced-near-complement} contains the Proposition~\ref{prop:bondareva-complement} weighting on the common applicability domain (setting $\lambda_S=1/(n-1)$ on $\mathcal{B}_{n-1}$, zero on $\mathcal{B}_{n-2}$ recovers Proposition~\ref{prop:bondareva-complement}; see Section~\ref{ssec:empty}), specifically including the 9 ``balanced-near-complement'' (best label) cases listed above.}
\label{tab:thm5}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lcc|c@{}}
...
Balanced-complement (Proposition~\ref{prop:bondareva-complement}, best label)                 & 11 &   0 &  11 \\
Balanced-near-complement (Proposition~\ref{prop:balanced-near-complement}, best label)         &  9 &   0 &   9 \\
...
Every instance classified by any of the five mechanisms as ``empty'' is indeed empty (no false positives across 525 policy-instance pairs). The three complement-based mechanisms cover 57 of 67 NN empties; Proposition~\ref{prop:partition-pair} certifies the 9 intermediate cases of Observation~\ref{obs:intermediate} (Supplementary~\S\ref{app:partition-pair-cases}), so the four thresholds jointly cover 66 of 67. Only the 1 near-complement residual (Remark~\ref{rem:near-complement}, $r\le r^{(\diamond)}_{\min}$ by $\approx 2\times 10^{-3}$) is uncovered analytically. Figure~\ref{fig:r-rss} plots $r$ versus $r^{\ast\ast}$: instances strictly above the diagonal ($r>r^{\ast\ast}$) are the single-complement mechanism, while balanced-complement, balanced-near-complement, near-complement-residual, and intermediate cases lie at or below the diagonal and are covered by their respective certificates. The sharpness of $r^{\ast\ast}$ and $r^{\ast\ast\ast}$ relative to $r^{\ast}$ is illustrated in Supplementary Figure~\ref{fig:rhist} (\S\ref{app:sharpness}).

\begin{figure}[t]
\centering
\includegraphics[width=0.65\textwidth]{fig2_r_vs_rstar.pdf}
\caption{Empirical test of Theorem~\ref{thm:empty-core} and Proposition~\ref{prop:bondareva-complement}. The dashed line is $r=r^{\ast\ast}$; 37 instances strictly above the diagonal have an empty Core (red $\times$, single-complement mechanism). Of the 10 additional empty-Core instances plotted here as near-complement (purple diamond), 9 are certified by Proposition~\ref{prop:balanced-near-complement} (balanced-near-complement threshold on $B_{n-1}\cup B_{n-2}$) and 1 remains covered only by the first-stage Core LP (Remark~\ref{rem:near-complement} residual); the 11 balanced-complement cases covered by Proposition~\ref{prop:bondareva-complement} (orange square) sit at or below the diagonal. The nine intermediate-coalition cases of Observation~\ref{obs:intermediate} have $r^{\ast\ast}$ undefined and are not plotted. See Section~\ref{ssec:cor16} for the full five-mechanism decomposition. The legend's ``Core non-empty (n=38)'' counts only the 96 instances on which $r^{\ast\ast}$ is defined; pooled with the 9 intermediate-coalition cases and the remaining nonempty instances on which $r^{\ast\ast}$ is undefined, this is consistent with the overall 108 nonempty Cores reported in Table~\ref{tab:thm5}.}
\label{fig:r-rss}
\end{figure}
...
\paragraph{Analytic sharpness of Corollary~\ref{cor:cor52}.} The analytic content of Corollary~\ref{cor:cor52} is that all three complement-based mechanisms---single (Theorem~\ref{thm:empty-core}), balanced (Proposition~\ref{prop:bondareva-complement}), and balanced-near-complement (Proposition~\ref{prop:balanced-near-complement})---are blocked whenever the peak queue satisfies $k<n-1$, because each hypothesis requires at least one complement $N\setminus\{i\}$ to be feasible, and feasibility forces $|U_t|=n-1$ at some $t$. Empirically this holds without exception in the 175-instance grid: no NN (or CI, BR) instance with $k<n-1$ satisfies $r>r^{\ast\ast}$, $r>r^{\ast\ast\ast}$, or $r>r^{(\diamond)}_{\min}$ (false-positive count is zero for all three thresholds across all 525 policy-instance pairs).
...
\end{itemize}
A per-pattern breakdown across $n\in\{5,7,10,12,15\}$ appears in Supplementary Materials \S S1 (Supplementary Figure~S1).

\paragraph{Nine empty Cores at $k<n-1$: intermediate-coalition mechanism.} The 9 empty-Core instances in the $k<n-1$ regime are all certified by the intermediate-coalition mechanism of Observation~\ref{obs:intermediate}. All 9 lie in $\text{B}_{\text{medium}}$ (7 cases) or E (2 cases), have $k$ between $n-5$ and $n-2$, and realize $r\in[1.33,1.61]$. Since both Theorem~\ref{thm:empty-core} and Proposition~\ref{prop:bondareva-complement} are vacuous in the $k<n-1$ regime, the 9 cases constitute direct evidence of a third mechanism operating outside the complement-coalition framework. Table~\ref{tab:intermediate} summarises these instances; full binding-size distributions are deferred to Supplementary Materials \S S3. For the 9 cases the partition pairs were obtained from the dual support of the first-stage Core LP (Supplementary~\S\ref{app:partition-pair-cases}); a predictive analytic characterization of which arrival-service realizations admit such a partition pair, derived from queueing-theoretic primitives rather than from after-the-fact LP duals, is open (Section~\ref{sec:conclusion}, Limitation (iv)).
...
\paragraph{Five-way decomposition of 67 empty Cores (NN).} The 67 empty Cores decompose into five mechanisms:
\begin{itemize}
  ...
  \item Balanced-near-complement (Proposition~\ref{prop:balanced-near-complement}): \textbf{9} instances, all at $k=n$.
  \item Near-complement residual (Remark~\ref{rem:near-complement}, no threshold fires): \textbf{1} instance ($n=15$, Pattern A, seed 42; $r=1.0475$, $r^{(\diamond)}_{\min}=1.0495$), at $k=n$.
  \item Intermediate (Observation~\ref{obs:intermediate}): \textbf{9} instances, all at $k<n-1$.
\end{itemize}
Two regimes emerge: the saturated regime $k=n$ concentrates the four complement-based mechanisms (single + balanced + balanced-near-complement + near-complement-residual: 58 of 58 empty cases at $k=n$), while the shallow-queue regime $k<n-1$ is governed exclusively by the intermediate mechanism. These regimes are empirically disjoint in our grid.
...
\caption{Temporal Core existence rate stratified by peak-queue regime $k$. The complement-coalition mechanisms (Theorem~\ref{thm:empty-core}, Proposition~\ref{prop:bondareva-complement}, Proposition~\ref{prop:balanced-near-complement}) all require $k\ge n-1$. Empirically, 58 of the 67 empty Cores concentrate at $k=n$ via single / balanced / balanced-near-complement / near-complement-residual mechanisms; the remaining 9 occur at $k<n-1$ via the intermediate-coalition mechanism of Observation~\ref{obs:intermediate}, all in patterns $\text{B}_{\text{medium}}$ or E.}
\label{fig:core-k}
\end{figure}
...
Three patterns stand out. Simultaneous arrivals (A, $\rho=\infty$) yield the lowest Core-existence rate ($36\%$), consistent with $\F=2^N\setminus\{\emptyset\}$ maximizing the number of Core constraints. Light-load sequential $\text{B}_{\text{light}}$ ($\rho=0.5$) leaves $|\F|/(2^n-1)$ at only $0.077$ and renders Theorem~\ref{thm:empty-core} inapplicable in all 25 instances. Mid-dispersal patterns D and E also keep $|\F|$ small ($0.196$ and $0.273$ respectively), with Theorem~\ref{thm:empty-core} applicable in only $3/25$ and $5/25$ instances and firing in $0/25$ in both. The clustered pattern C and the heavy-load $\text{B}_{\text{heavy}}$ have $\rho\ge 2$ and near-saturated feasibility ($\overline{|\F|/(2^n-1)}=0.985$ each), placing them in the regime where Theorem~\ref{thm:empty-core} applies to nearly every instance and fires in $10/25$ (C) and $15/25$ ($\text{B}_{\text{heavy}}$).
...
Empirically, on the NN grid $\bar r^{\ast\ast}=1.223$ and $\bar r^{\ast\ast\ast}=1.096$ versus $\bar r^{\ast}=4.351$; the sharpness factors are $r^{\ast}/r^{\ast\ast} \approx 3.56$ and $r^{\ast}/r^{\ast\ast\ast} \approx 3.97$. Because the realized ratio $\bar r = 1.312$ is close to $r^{\ast\ast}$ and $r^{\ast\ast\ast}$ but far below $r^{\ast}$, Theorem~\ref{thm:empty-core} and Proposition~\ref{prop:bondareva-complement} are operationally informative whereas Remark~\ref{rem:rstar} almost never fires. The classical bound grows with $n$ because $r^{\ast} = \sum_i c(\{i\})/\cstar(N)$ has a numerator scaling linearly in $n$ while $\cstar(N)$ scales like $\sqrt{n}$ on uniform planar instances; $r^{\ast\ast}$ and $r^{\ast\ast\ast}$, in contrast, are pinned near $1$ because complement coalitions $N\setminus\{i\}$ nearly partition the grand tour. This explains why the gap between the two widens with $n$ and quantifies the practical advance of the temporal analysis. Supplementary~\S\ref{app:sharpness} (Supplementary Figure~\ref{fig:rhist}) plots the three threshold distributions side by side.

\subsection{Robustness to dispatch policy}
\label{ssec:policy}

The three sufficient conditions of \S\ref{sec:theory} are policy-independent in their threshold values---which depend only on TSP costs and the arrival sequence, not on service times---and applicability counts differ by at most one across nearest-neighbor (NN), cheapest-insertion (CI), and batch re-optimization (BR) dispatch over $175\times 3 = 525$ policy-instance pairs; all three thresholds preserve the no-false-positives guarantee, while the intermediate-coalition mechanism of Observation~\ref{obs:intermediate}---observed only under NN---disappears under both CI and BR. Detailed per-policy results, including the dispatch-policy contingency table and the empty-mechanism breakdown, appear in Supplementary~\S S5.
...
{\footnotesize $^{\dagger}$ Core LP not enumerated at this scale; Theorem~\ref{thm:empty-core} fires in $4/5$ seeds at $n=30$ and $5/5$ at $n=50$; the one non-firing seed at $n=30$ (seed 123) is certified by a restricted Core LP (Supplementary Materials \S S3). $^{\ddagger}$ At $n=20$, Theorem~\ref{thm:empty-core} is vacuous ($r\le r^{\ast\ast}$) at seeds 99 and 123 and fires at seeds 7, 42, 256; at $n=30$, it is vacuous at seed 123 only. Core emptiness for the vacuous seeds is certified by the near-complement mechanism (Remark~\ref{rem:near-complement}, Section~\ref{ssec:cor16}). $^{\S}$ No intermediate-coalition violation detected in the sampled restricted LP of $10^{4}$ coalitions per instance; this does \emph{not} certify full Core nonemptiness (per-draw detection fraction $\approx 10^{-2}, 10^{-5}, 10^{-11}$ at $n=20, 30, 50$ respectively). The structural statement (complement mechanisms blocked by Corollary~\ref{cor:cor52}) is analytically sharp; the no-violation observation is one-sided. At $n>15$ the Core-existence judgment is made via the restricted LP of Supplementary Materials \S S3 rather than full $2^n-1$ enumeration.}
\end{table}

Four findings emerge.

\paragraph{Finding 1: $\bar r^{\ast\ast}$ decays with $n$, confirming Theorem~\ref{thm:asymptotic}.} For pattern A, $\bar r^{\ast\ast}$ drops from $1.181$ at $n=15$ (Table~\ref{tab:pattern}) to $1.119$, $1.096$, $1.068$ at $n=20, 30, 50$, respectively. Pattern C exhibits the same decay ($\text{B}_{\text{medium}}$ has no feasible complement at any scale, so $\bar r^{\ast\ast}$ is undefined). The observed rate of decrease is consistent with the $O(n^{-1/2})$ bound of Theorem~\ref{thm:asymptotic} (and within the $O(n^{-1})$ improvement of Remark~\ref{rem:asymptotic-tighter} for the simultaneous-arrival case A). A log-log regression of $\bar r^{\ast\ast}-1$ on $n$ for Pattern A yields slope $-0.54$ (R$^2=0.99$, eight per-$n$ mean points $n\in\{5,7,10,12,15,20,30,50\}$) and $-0.62$ (R$^2=1.00$, four per-$n$ mean points on the scale-up subset $n\in\{15,20,30,50\}$), close to Theorem~\ref{thm:asymptotic}'s $-1/2$ and partway toward Remark~\ref{rem:asymptotic-tighter}'s $-1$.

\paragraph{Finding 2: Pattern C activates the complement-mechanism as $n$ grows.} Pooled over $n\in\{5,7,10,12,15\}$, pattern C has $\bar r = 1.197$ marginally above $\bar r^{\ast\ast}=1.181$ (Table~\ref{tab:pattern}), with a $28\%$ Core-existence rate overall; at $n=15$ under NN all 5 instances are already empty (mean $r=1.238$ vs.\ mean $r^{\ast\ast}=1.145$). By $n=20$ the gap widens substantially ($\bar r = 1.335$ vs.\ $\bar r^{\ast\ast} = 1.119$), and all 5 test instances are empty; the same holds at $n=30, 50$. Two concurrent effects are at work: $\bar r$ increases (the NN competitive ratio degrades with more customers and the clustered zig-zag accumulates realized waiting), while $\bar r^{\ast\ast}$ decreases by Theorem~\ref{thm:asymptotic}. Clustered patterns thus shift from marginal to strong complement-mechanism activation as $n$ grows.
...
\paragraph{Detection-limit warning ($n=50$ specifically).} At $n=50$ the per-draw detection probability for any single binding intermediate-size coalition is $\sim 10^{-11}$, so the absence of violations in our $K=10^{4}$ sampled LP carries essentially no evidential weight: Finding~3(b) at $n=50$ should be read as a placeholder for ``intractable to test.'' The caveat is weaker at $n=30$ ($\sim 10^{-5}$ per draw, joint detection still under $10\%$) and operationally negligible at $n=20$. Only the structural claim 3(a) carries asymptotic strength.

\paragraph{Finding 4: Near-complement mechanism at seed 123 across scales.} Seed 123 under Pattern~A exhibits a two-tier certification pattern consistent with Theorem~\ref{thm:asymptotic}'s $O(n^{-1/2})$ tightening of $\bar r^{\ast\ast}$:
\begin{itemize}
  \item \emph{At $n=20$ and $n=30$}, Theorem~\ref{thm:empty-core} is vacuous ($r=1.077<r^{\ast\ast}=1.112$ and $r=1.038<r^{\ast\ast}=1.088$ respectively). A restricted Core LP over $10{,}000$ sampled coalitions certifies Core emptiness with binding constraints concentrated at sizes $n-1$ and $n-2$ simultaneously, consistent with the near-complement mechanism of Remark~\ref{rem:near-complement}. These three cases constitute the Supplementary Materials \S S3 case study.
  \item \emph{At $n=50$}, Theorem~\ref{thm:empty-core} directly fires ($r=1.306>r^{\ast\ast}=1.081$), analytically certifying Core emptiness without requiring restricted-LP evidence. The threshold $r^{\ast\ast}$ at $n=50$ is small enough (by Theorem~\ref{thm:asymptotic}'s decay) that the realized competitive ratio clears it, so the analytic certificate takes over and the near-complement auxiliary no longer needs to be invoked.
\end{itemize}
By contrast, a clean Theorem-\ref{thm:empty-core}-fires instance at smaller scale (Pattern A, $n=30$, seed 42) binds only at size $n-1$ (Section~\ref{ssec:cor16}). This transition from restricted-LP certification at the smaller seed-123 scales ($n=20,30$) to analytical certification at $n=50$ is the natural consequence of $r^{\ast\ast}\to 1$; seed 123 is not itself anomalous in the asymptotic limit, only uncovered by Theorem~\ref{thm:empty-core} at finite $n$ below the transition.

\paragraph{Intractability of pattern A at $n\ge 30$.} For pattern A, $|\F| = 2^n-1$, so the Core LP has $\sim\!10^9$ and $\sim\!10^{15}$ constraints at $n=30$ and $n=50$ respectively, well beyond enumeration. Theorem~\ref{thm:empty-core} nevertheless certifies Core emptiness \emph{a priori} in $12$ of the $15$ seed--size combinations: every complement coalition is feasible, so $r^{\ast\ast}$ is well-defined, and numerically $r>r^{\ast\ast}$ holds in $3/5$ seeds at $n=20$, $4/5$ at $n=30$, and $5/5$ at $n=50$ (Table~\ref{tab:scaleup}). The three exceptions---seeds 99 and 123 at $n=20$, and seed 123 at $n=30$---have $r\le r^{\ast\ast}$; for these the restricted Core LP of Supplementary Materials \S S3 certifies emptiness (Finding~4 above). The intractability of pattern A therefore itself \emph{demonstrates} the $k<n-1$ versus $k=n$ divide identified in Corollary~\ref{cor:cor52}: the regime where the Core LP becomes infeasible to enumerate is precisely the regime where Theorem~\ref{thm:empty-core}---supplemented by the near-complement certificate at the few exceptional seeds---can certify emptiness without enumeration.

\paragraph{Summary.} The qualitative findings persist to $n=50$: the Core-empty tail at $n=15$ (pattern C) becomes near-certain by $n=20$ via $r^{\ast\ast}$ tightening (Theorem~\ref{thm:asymptotic}), while Corollary~\ref{cor:cor52}'s structural safeguard only strengthens ($\bar k<n-1$ analytically blocks complement mechanisms at every scale-up cell). The empirical companion for $\text{B}_{\text{medium}}$---no intermediate violation observed in our sampled LP---is one-sided and detection-power limited (Finding~3(b)); it does not bound the probability of empty Core. The reading is: one strong analytic conclusion (structural blocking) plus one weak empirical one (no sampled violation), consistent with the main-grid observation that intermediate empties vanish under CI/BR (\S\ref{ssec:policy}).

\section{Conclusion}
\label{sec:conclusion}

We introduced the Online Traveling Salesman Game and defined the Temporal Nucleolus as the lexicographic excess minimizer over the restricted family $\F$ of coalitions realizable under the online arrival process. The classical (static) TSG is recovered at the boundary case of simultaneous arrivals; the Temporal Core is empty whenever the competitive ratio exceeds any of three complement-based thresholds (Theorem~\ref{thm:empty-core}, Proposition~\ref{prop:bondareva-complement}, Proposition~\ref{prop:balanced-near-complement}) or the unifying partition-pair certificate of Proposition~\ref{prop:partition-pair}; a bounded waiting queue ($k<n-1$) structurally blocks the three complement-based mechanisms (Corollary~\ref{cor:cor52}), but not the partition-pair mechanism via non-singleton splits. On the 175-instance NN grid, the four analytic thresholds jointly certify 66 of 67 observed empty Cores with no false positives---1 near-complement residual completes the five-mechanism decomposition. The thresholds are sharper than the classical bound by $3.56\times$ ($\bar r^{\ast\ast}=1.223$) and $3.97\times$ ($\bar r^{\ast\ast\ast}=1.096$), making them the first operationally informative thresholds for cost-sharing stability in online routing.

\paragraph{Platform-design implication.} From a platform-design perspective, our results offer suggestive evidence that enforcing short service-time promises---equivalently, keeping the peak queue depth below $n-1$---structurally rules out all three complement-coalition mechanisms for emptying the Core. In the shallow-queue regime, the partition-pair mechanism of Proposition~\ref{prop:partition-pair} remains logically possible (Observation~\ref{obs:intermediate}), and is empirically observed only under nearest-neighbor dispatch: cheapest-insertion and batch re-optimization produce zero such cases in our grid, suggesting that even moderately informed dispatch policies mitigate this mechanism (Section~\ref{ssec:policy}).

\paragraph{Operational trade-offs.} The platform-design recommendation that bounding $k<n-1$ structurally blocks the complement-coalition mechanisms (Corollary~\ref{cor:cor52}) is not without operational cost. Achieving $k<n-1$ in practice requires either tighter service-time commitments (raising delivery-speed standards), increased fleet capacity (more vehicles serving simultaneously), or more frequent dispatch (reducing idle waiting at the depot); each imposes resource costs that the cooperative-game analysis here does not quantify. Moreover, $k$ is itself an endogenous outcome of the arrival rate and service rate distributions, not a free control variable: a platform with high request volume may face $k=n$ regimes even with aggressive dispatching. Whether the Core-stability gain from bounding $k$ is worth the operational cost is a platform-design judgment that depends on cost-of-defection considerations not modeled here. Combining the cooperative game with explicit fleet-capacity costs along the lines of \citet{ozener2008allocating} would clarify this trade-off.
...
\item \emph{Dispatch policy.} Theorem~\ref{thm:empty-core},
  Supplementary Proposition~\ref{prop:core-inheritance} (\S\ref{app:inheritance}), and
  Corollary~\ref{cor:cor52} use only the realized $(r,\F,k)$, hence are
  policy-independent; Section~\ref{ssec:policy} verifies this empirically
  across NN/CI/BR. Learning-based or forecasting-aware dispatch remains open.
\item \emph{Instance scale.} Section~\ref{ssec:scaleup} extends to
  $n\in\{20,30,50\}$ with LKH for $c^{\ast}(S)$ at $n>20$ (calibration error
  $0.000\%$ vs.\ Held--Karp at $n=15$, well below the minimum $r-r^{\ast\ast}$
  margin of $0.0159$). For $\text{B}_{\text{medium}}$ the complement
  mechanisms are structurally blocked at every scale (Corollary~\ref{cor:cor52});
  the intermediate-coalition mechanism is not flagged by our $K=10^4$ sampled
  LP, but the sampling fraction $K/(2^n-1)$ collapses with $n$, so the
  $\text{B}_{\text{medium}}$ nonemptiness claim is structural plus one-sided
  empirical, not a proof. Extending to $n\ge 100$, non-uniform geometries,
  heavy-tailed queueing, and full-$\F$ scale-up certification is open.
\item \emph{Near-complement residual.}
  Proposition~\ref{prop:balanced-near-complement} closes 9 of 10 main-grid
  cases; the residual ($n=15$, A, seed 42) misses by $\approx 2\times 10^{-3}$.
  Whether enlarging the balancing family to $B_{n-1}\cup B_{n-2}\cup B_{n-3}$
  or relaxing the efficiency equality to a covering inequality closes this gap
  is open (Supplementary \S S4 records the diagnostic).
\item \emph{Intermediate-coalition mechanism.}
  Proposition~\ref{prop:partition-pair} certifies the 9 cases of
  Observation~\ref{obs:intermediate} via dual support extraction
  (Supplementary~\S\ref{app:partition-pair-cases}). What remains open is a
  policy-aware predictive condition derived from queueing primitives
  (queue-depth distribution, service-time variability) rather than after-the-
  fact LP duals. The mechanism is also policy-sensitive: observed only under
  NN, absent under CI/BR.
\item \emph{Policy-dependence of $\F$.} Threshold \emph{values}
  ($r^{\ast\ast}, r^{\ast\ast\ast}, r^{(\diamond)}_{\min}, r^{(P)}_{\min}$)
  are policy-independent; the family $\F$ itself depends on realized service
  times and hence on the dispatch policy. The policy-induced framing adopted
  here is operationally realistic; a counterfactual framing in which $\F$
  depends only on arrival times is more robust to policy changes. Systematic
  comparison is left for future work.
\end{enumerate}

\emph{Scope demarcation and Online VRG.} The single-vehicle scope is a deliberate methodological choice: it isolates the temporal coalition-feasibility structure of $\F$ from fleet-assignment confounds, and the three complement-based sufficient conditions rely on $c(S)$ being a single TSP tour cost for which the insertion identity $c(\{i\})+c(N\setminus\{i\})\ge\cstar(N)$ holds. An \emph{Online Vehicle Routing Game} combining $\F$ with multi-vehicle capacity \citep{gothe1996vrg,engevall2004heterogeneous} requires redefining $c(S)$ jointly with capacity and a fleet-assignment rule, under which that insertion identity can fail; this opens a separate agenda (capacitated coalition costs, fleet-level balancing, A3-style rolling-horizon consistency \citep{lehrer2013dynamic,kimms2016rolling}) that a companion manuscript will develop.
...
\setlength{\bibsep}{1pt plus 0.2ex minus 0.2ex}
\begin{spacing}{1.0}
