...
\label{ex:solomon}
We illustrate the Online TSG on the first five customers of the Solomon C101 benchmark \citep{solomon1987benchmarks}. Depot is at $(35, 35)$ and coordinates/ready times are as in Table~\ref{tab:solomon}. Coordinates and ready times follow Solomon's original units (\citealp{solomon1987benchmarks}); we treat the coordinate plane as Euclidean with unit travel cost and the time axis with vehicle speed normalized to one travel-cost unit per time unit. The online policy is nearest-neighbor departing from the depot at $t=0$.
