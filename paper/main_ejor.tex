...
$r^{\ast\ast}$ & $1+\min_{i:\,N\setminus\{i\}\in\F}\delta_i/\cstar(N)$: tight threshold (Thm.~\ref{thm:empty-core}); undefined if no such $i$ exists; computable a priori from coordinates \\
$r^{\ast\ast\ast}$ & $\bigl(\sum_{i\in N} c(N\setminus\{i\})\bigr)/\bigl((n-1)\,\cstar(N)\bigr)$: balanced-complement threshold (Prop.~\ref{prop:bondareva-complement}); requires $B_{n-1}\subseteq\F$; computable a priori from coordinates \\
$r^{(\diamond)}_{\min}$ & LP optimum of Bondareva--Shapley over $B_{n-1}\cup B_{n-2}$ (Prop.~\ref{prop:balanced-near-complement}); requires $B_{n-1}\cup B_{n-2}\subseteq\F$; post-hoc LP solve required \\
$r^{(P)}_{\min}$ & Partition-pair threshold $\min_{(S,N\setminus S)\in\F^{2}}\,(c(S)+c(N\setminus S))/\cstar(N)$ (Prop.~\ref{prop:partition-pair}); post-hoc, via partition enumeration or Core-LP dual \\
...
\paragraph{Computability of the four sufficient conditions.} The thresholds below differ in computational character. Theorem~\ref{thm:empty-core}'s $r^{\ast\ast}$ and Proposition~\ref{prop:bondareva-complement}'s $r^{\ast\ast\ast}$ depend only on $\{c(\{i\}), c(N\setminus\{i\}), \cstar(N)\}$ and on the realized $\F$; they are computable \emph{a priori} from coordinates and arrival--service windows. Proposition~\ref{prop:balanced-near-complement}'s $r^{(\diamond)}_{\min}$ requires an LP solve per instance on the family $B_{n-1}\cup B_{n-2}$. Proposition~\ref{prop:partition-pair}'s $r^{(P)}_{\min}$ requires either enumeration over feasible partition pairs or extraction from the first-stage Core-LP dual; we therefore use it as a \emph{post-hoc certificate}. Operationally, $r^{\ast\ast}$ and $r^{\ast\ast\ast}$ are the informative thresholds; $r^{(\diamond)}$ and $r^{(P)}$ close the empirical residuals (Section~\ref{ssec:cor16}, Observation~\ref{obs:intermediate}).

\begin{table}[h]
\centering
\caption{Hierarchy of sufficient conditions for an empty Temporal Core, in ascending sharpness. Statements and proofs follow in Theorems~\ref{thm:empty-core},~\ref{thm:asymptotic} and Propositions~\ref{prop:bondareva-complement},~\ref{prop:balanced-near-complement},~\ref{prop:partition-pair}.}
...
Single-complement (Thm.~\ref{thm:empty-core}) & $r > r^{\ast\ast}$ & $\exists\, i:\, N\setminus\{i\}\in\F$ & a priori & $\sim 3.56$ \\
Balanced-complement (Prop.~\ref{prop:bondareva-complement}) & $r > r^{\ast\ast\ast}$ & $B_{n-1}\subseteq\F$ & a priori & $\sim 3.97$ \\
Balanced-near-complement (Prop.~\ref{prop:balanced-near-complement}) & $r > r^{(\diamond)}_{\min}$ & $B_{n-1}\cup B_{n-2}\subseteq\F$ & post-hoc LP & --- \\
General partition-pair (Prop.~\ref{prop:partition-pair}) & $r > r^{(P)}_{\min}$ & $\exists\,(S,N\setminus S)\in\F^{2}$ & post-hoc LP/enum & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\]
where $\tau$ is the mean inter-arrival interval and $\beta_{\mathrm{BHH}}\approx 0.7124$ is the Beardwood--Halton--Hammersley constant (the asymptotic per-customer Euclidean TSP edge length under the dimensionless $L=\sqrt{n}$ convention with unit density and $v=1$). This convention makes the pattern generation $\tau=\beta_{\mathrm{BHH}}/\rho$ self-consistent and renders $\rho$ scale-invariant under joint $(L,v,\tau)$ rescaling. We verify this invariance empirically to machine precision in Supplementary Materials Section~S2.
...
Three regimes emerge. Patterns with $\rho\ge 2$ and saturated arrival timing (A, $\text{B}_{\text{heavy}}$, C) reach near-full feasibility ($\overline{|\F|/(2^n-1)}\ge 0.985$ for $\text{B}_{\text{heavy}}$ and C; $|\F|=2^n-1$ for A) and place Theorem~\ref{thm:empty-core} in the applicable regime for nearly every instance, firing in $9/25$, $15/25$, and $10/25$ respectively. Sparse-feasibility regimes ($\text{B}_{\text{light}}$, D, E) keep $\overline{|\F|/(2^n-1)}\le 0.27$ and render Theorem~\ref{thm:empty-core} either inapplicable or non-firing; in these regimes the mean competitive ratio rises ($\bar r\in[1.33,1.69]$) because the online tour detours to reach customers as they arrive while $\cstar(N)$ reflects the offline optimum, but Corollary~\ref{cor:cor52} blocks every complement-based mechanism and the 2 empty Cores observed in pattern E arise solely via the intermediate-coalition mechanism of Observation~\ref{obs:intermediate}. Per-pattern statistics are in Table~\ref{tab:pattern}.

\paragraph{Load parameter $\rho$ alone does not determine Core stability.} Patterns $\text{B}_{\text{medium}}$ (uniform sequential) and C (clustered) share $\rho=2$ but behave qualitatively differently: $\text{B}_{\text{medium}}$ is complement-mechanism-free in $22/25$ instances (Core rate $0.56$), while C activates Theorem~\ref{thm:empty-core} in $24/25$ (Core rate $0.28$). The structural difference in arrival geometry alone accounts for this gap; $\rho$ is necessary but not sufficient to characterize Core stability.
...
\paragraph{Finding 4: Near-complement-to-Theorem-9 transition at seed 123.} Seed 123 under Pattern~A exhibits a transition consistent with Theorem~\ref{thm:asymptotic}'s $O(n^{-1/2})$ tightening of $r^{\ast\ast}$: at $n=20,30$, $r<r^{\ast\ast}$ and Core emptiness is certified by the near-complement mechanism (Remark~\ref{rem:near-complement}) via a restricted Core LP; at $n=50$, $r^{\ast\ast}$ has tightened sufficiently that Theorem~\ref{thm:empty-core} fires directly. Detailed per-seed binding-size distributions and the restricted-LP methodology are in Supplementary Materials Section~S3.

\paragraph{Intractability of Pattern A at $n\ge 30$.} For Pattern~A, $|\F|=2^n-1$ ($\sim 10^9$ at $n=30$, $\sim 10^{15}$ at $n=50$), beyond enumeration. Theorem~\ref{thm:empty-core} nevertheless certifies emptiness in 12 of 15 seed--size combinations a priori (Table~\ref{tab:scaleup}); the three exceptions (seeds 99, 123 at $n=20$; seed 123 at $n=30$) are certified by the restricted Core LP of Supplementary Materials Section~S3 (Finding~4 above). The intractability regime is thus precisely where Theorem~\ref{thm:empty-core} (with Remark~\ref{rem:near-complement} backup) suffices without enumeration.
...
\item \emph{Dispatch policy.} Theorems and corollaries depend on the
  ...
  documented in Section~\ref{ssec:policy}; learning-based or
  forecasting-aware policies remain open.
...
\item \emph{Near-complement residual.}
  Proposition~\ref{prop:balanced-near-complement} closes 9 of 10
...
\item \emph{Intermediate-coalition mechanism.}
  ...
  rather than after-the-fact LP duals, is open. The mechanism is also
  policy-sensitive: observed only under NN, absent under CI/BR.
