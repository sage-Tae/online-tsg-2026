## v2_5_11 (2026-05-06) — Pre-submission external-review fixes (factual + ethics + bib + cover letter)

External review of v2.5.10 PDF flagged six items — three factual (A1–A3),
one ethics-disclosure gap (B1), two bib DOI errors of the AAMAS-no-DOI
pattern (C1–C2), and a cover-letter addressee anonymization (D1).

**A1.** §6.3 "$k$ between $n-5$ and $n-2$" → "$n-3$ and $n-2$" (Table~5
verification: $n-k\in\{2,3\}$ across all 9 intermediate cases).

**A2.** §6.3 and Observation~15 statement (line 378): realized
competitive-ratio range "$r\in[1.33,1.61]$" → "$r\in[1.326,1.610]$".
Lower endpoint 1.326 is strictly below the previous 1.33 rounding;
Table 5 data show min 1.326, max 1.610.

**A3.** §6.4 $\text{B}_{\text{medium}}$ counting reconciled. Previous
text said "Theorem 9 fires in 3, plus 7 intermediate, Core rate 0.56"
but $25-3-7=15$ would give Core rate 0.60. Table 6 row shows Prop 10
fires in 4 (one beyond Thm 9's 3). Sentence rewritten to make the
bookkeeping explicit: $3+1+7=11$ empty out of 25, Core rate $0.56$.
After page-fitting trim, the final sentence reads "$\text{B}_{\text{medium}}$
\dots: Theorem 9 fires in $3/25$, Proposition 10 in one further case,
and the intermediate mechanism in $7$ more, totaling $11/25$ empty
(Core rate $0.56$)."

**B1.** New `\paragraph*{Declaration of generative AI in scientific
writing.}` block added to Acknowledgements (immediately before "Data
and code availability"). Elsevier-policy compliant: states use
(language editing, analysis-script drafting), states review/edit, and
states full author responsibility. Compressed 30-word version (vs
Elsevier's verbose ~52-word template) used to fit page budget while
preserving all three policy elements.

**C1.** `ge2024incentives`: removed wrong-venue DOI `10.24963/ijcai.2024/1211`
(IJCAI prefix; AAMAS proceedings have no DOI). Initially replaced with
IFAAMAS PDF URL, then with `note = {IFAAMAS}`, finally removed entirely.
Bare AAMAS proceedings citation (venue + pages + year) is sufficient
for verification — analogous to `kopelowitz1967computation`'s
documented exception. AAMAS proceedings is now a formally documented
no-DOI venue alongside the 1967 tech report.

**C2.** `zhang2025incentives`: same treatment — removed unverified
DOI prefix `10.65109/yqsz4619`, no replacement URL/note retained.

**D1.** Cover letter anonymized: addressee "Professor Roman
Słowiński, Co-ordinating Editor-in-Chief" → "Editor-in-Chief";
salutation "Dear Professor Słowiński" → "Dear Editor". Authors do not
know the handling editor in advance of submission. `cover_letter_ejor.pdf`
regenerated via `pandoc … --pdf-engine=xelatex`.

**E3.** Highlights bullet 2 rewritten: "Cooperative Core empties under
realized arrivals; Temporal Nucleolus stabilizes it." → "Temporal
Nucleolus provides a unique allocation even when the Core is empty."
(76 chars; preserves 85-char cap and abbreviation-free rule.)

**Page-budget management.** B1 added ~3 lines and A3 added ~1 line,
pushing v2.5.10's 30-page main into 31. C1/C2's IFAAMAS URL replacement
(longer than the original wrong DOIs) compounded with another ~2 lines
of overflow. After trimming the §5.3 last sentence, removing C1/C2
URL/note fields entirely, and condensing the §6.4 Bmedium and §6.6
Robustness opening sentences, the manuscript returned to 30 pages.
All factual claims, theorem statements, proofs, tables, figures, and
numerical results are unchanged. Trim summary:
- §5.3 Discussion: removed last sentence ("Short service-time promises
  and informed dispatch are therefore complementary levers...");
  preceding sentence already conveys the dispatch-policy point.
- §6.4 $\text{B}_{\text{medium}}$ sentence: tightened from 37 to 24
  words; A3's $3+1+7=11$ accounting now expressed compactly.
- §6.6 Robustness opening: 49 → 30 words; NN/CI/BR expansion dropped
  (already defined in Table 6 caption); "observed only under NN"
  parenthetical dropped (re-stated in §6.7 Findings).

**Page counts.** main 30 (preserved from v2.5.10) / supp 13 (unchanged).

**Manual actions still required.**
- Git commit/tag: working directory in this environment is not a git
  repository (`Is a git repository: false`). User to run
  `git add … && git commit -m "v2.5.11: pre-submission external-review fixes" && git tag v2.5.11`
  in their local repo clone.

## v2_5_10 (2026-05-06) — Acknowledgements section + DOI completion

EJOR submission Editorial Manager flow exposed two missing items in v2.5.9.

**Acknowledgements section.** The KITECH grant statement was present in
the cover letter but missing from `main_ejor.tex`. Added a short
\section*{Acknowledgements} block immediately before "Data and code
availability":

> "This work was supported by the Korea Institute of Industrial
> Technology under Grant No. EH-26-0002."

This makes the manuscript self-contained for funding disclosure (the
cover letter is not visible to reviewers and is stripped at production)
and lets us truthfully confirm the EM funding-declaration question.

**Reference DOI completion.** v2.5.9 had 4 of 46 entries without DOI:
- `arroyo2024vrptw` → DOI: 10.5282/jums/v9i1pp1241-1268 (Junior Management Science 9(1))
- `aziz2025participation` → DOI: 10.48550/arXiv.2502.19791 (arXiv standard)
- `goyal2025temporal` → DOI: 10.48550/arXiv.2510.11255 (arXiv standard)
- `kopelowitz1967computation` — Hebrew University Research Memorandum
  RM-31, 1967; pre-DOI grey literature; no DOI exists. This is a
  genuine exception and consistent with EM's "all papers" wording
  (papers for which a DOI exists).

After this fix, 45 of 46 references have DOI/ISBN (42 with DOI + 3 with
ISBN); the one exception is documented above. This lets us truthfully
confirm the EM academic-misconduct/DOI question.

No theorem statement, proof, table, figure, abstract, or section name
modified. Page count: main 30 / supp 13 (unchanged). All v2.5.x
carry-over compliance items (highlights md5 fe8c95..., ref styling,
section names, abstract narrative, body acronyms, figure 1
five-mechanism) preserved.

## v2_5_9 (2026-05-06) — Figure 1 image regenerated with corrected title

The plot title in `figures/fig2_r_vs_rstar.pdf` was updated from
"four-mechanism overlay" to "five-mechanism overlay" to match the body
text and caption. The legend remains unchanged (Single / Balanced /
Near-complement / Core non-empty); the 5th mechanism (intermediate)
continues to be shown as a plot annotation rather than a legend entry,
which is faithful to the underlying data structure (legend is keyed
to applicable rows; intermediate cases have $r^{**}$ undefined and so
appear in the annotation only).

**Mechanism of fix.** The legacy title in `make_figures_v3.py` (located
at `~/Library/CloudStorage/.../TSG_agent/code/figures/make_figures_v3.py`,
the most recent script copy on the user's Mac) was patched at line 85
with a one-word replacement (`four-mechanism` → `five-mechanism`) and
the script re-run from the original CSV inputs at
`../experiments/logs/policy_comparison_v2_full.csv` (525 rows; 175 NN).
The script's reported counts confirm the v2.5.x narrative: 96
applicable rows, 37 single-complement (Thm 9), 11 balanced-complement
(Prop 10), 10 near-complement, 9 intermediate (Obs 15, $r^{**}$
undefined), 38 nonempty-applicable. The regenerated PDF was copied
into `figures/fig2_r_vs_rstar.pdf`. Other five figures (fig1, fig3_*,
fig4, fig5) were left byte-identical to v2.5.8 (md5s confirmed).

fig2 md5: `232aa454...` → `1ab58af6...`. main_ejor.pdf rebuilt to embed
the new image. Page count: main 30 / supp 13 (unchanged). Theorem
numbering, abstract narrative form, body acronym definitions, section
names, ref styling, URL, and highlights all preserved from v2.5.8.
Editorial Manager submission checklist unchanged.

## v2_5_8 (2026-05-06) — Final ref-styling polish; Figure 1 regeneration deferred

**Issue D2 (completion).** v2.5.7 normalized only 8 of the
section-reference constructs (the conservative `Supplementary~\S\ref{...}`
pattern). v2.5.8 normalizes all remaining `\S` section references:

- 10 `\S\ref{...}` patterns → `Section~\ref{...}`
- 13 `\S S?` literals (e.g., `\S S2`, `\S S3`, ...) → `Section~S?`
- 8 carryover `Supplementary, \ref{...}` from v2.5.7 (which rendered as
  the awkward "Supplementary, X.X") → `Supplementary Section~\ref{...}`

Total: 31 cross-reference normalizations. Footnote-symbol uses (`$\S$`,
`$^{\S}$`) preserved (regex tested to exclude math-mode contexts). All
references compile and resolve identically; only the rendered styling
changes from "§<X>" to "Section~<X>". `Section~\ref{}` total in
main_ejor.tex grew from 6 (v2.5.7) to 54 (v2.5.8).

**Issue J — Figure 1 regeneration: SKIPPED.** The plot title in
`figures/fig2_r_vs_rstar.pdf` still reads "four-mechanism overlay"
(legacy text predating Proposition 14). Regenerating the figure
requires the source script (expected at `code/figures/make_figures_v3.py`),
which is not present in this submission tree — there is no `code/`
directory in `TSG_ejor_resubmission_v2_5_0/`. The script lives in the
GitHub repo's source clone, not in the EJOR submission package. The
body and caption correctly say "five-mechanism"; a reviewer may flag
the image-vs-caption inconsistency as a minor revision comment. Defer
to a future revision once the figure-generation script is co-located
with the submission tree (or the user runs the regeneration locally
from the GitHub clone).

No theorem statement, proof, table, or other figure modified. Page
count: main 30 / supp 13 (unchanged). Abstract narrative form unchanged
(242 words). All earlier compliance items (highlights file md5
`fe8c95fe...`, body acronym definitions, URL, KITECH affiliation)
preserved. 0 broken refs.

## v2_5_7 (2026-05-06) — EJOR style alignment (eight stylistic fixes)

After comparison against recent EJOR cooperative-game and routing papers
(Lyu et al. 2025, Martin-Iradi et al. 2022, Engevall et al. 2004, etc.),
eight stylistic divergences from EJOR convention were corrected. No
theorem statement, proof, table, figure data, or numerical result modified.

**Section names normalized to EJOR convention:**
- §2 "Related Work" → "Literature review"
- §6 "Computational Experiments" → "Computational study"
- §7 "Conclusion" → "Conclusions and future research"

**Cross-reference styling normalized:** the eight occurrences of
`Supplementary~\S\ref{...}` were normalized to `Supplementary, \ref{...}`.
The ten remaining `\S\ref{...}` parentheticals (in-paper section
references and Supplementary-context refs already inside parentheses)
compile and render consistently and were left unchanged per a
conservative directive — better to keep working refs than break
compilation.

**Three NeurIPS-style "Departure." paragraph headers in §2 dissolved**
into natural prose ("Our setting differs from these studies in...",
"Our work introduces a different source of restriction...", "Our work
differs from this online-game literature in two respects.").

**§3.2 "Ex-post, policy-induced character of $\F$" paragraph header
dissolved** into natural prose; the semicolon-and-clause was split into
two sentences and the parenthetical "Limitation (v)" was dropped since
the Conclusion enumeration is local-numbered.

**§5.3 Discussion enriched** from a 3-bullet itemize to two prose
paragraphs articulating (1) the structural parameter interactions and
(2) the role of the dispatch policy. All original claims preserved;
additional connection to Section~\ref{ssec:policy} added.

**Introduction Contributions promoted** from `\paragraph{}` to
`\subsection{Contributions}`. C1–C4 itemize unchanged; the operational
postamble and `\paragraph{Structure.}` retained.

Net page-count effect: main 30 → 30 (no change; prose additions in E/F/G
were balanced by header removals and tighter formatting). Supplementary
unchanged at 13. URL still `online-tsg-2026`. Highlights file
byte-identical to v2.5.5 (md5 fe8c95fe...). Theorem numbering preserved.
0 broken refs.

## v2_5_6 (2026-05-06) — Abstract: replace experimental ledger with narrative summary

EJOR-compliant in v2.5.5, but the last three sentences of the abstract
were in results-table form, dumping raw counts (175 / 67 / 57 / 9 / 525 /
50) into the abstract. Replaced with a two-sentence narrative summary:

- "Synthetic experiments validate the framework on three fronts: the
  analytic thresholds are sharp on the realized empty Cores with no false
  positives; the structural blocking persists across dispatch policies;
  and the asymptotic tightening agrees with the predicted square-root
  rate at moderate sizes."
- "Operationally, keeping the peak queue bounded suffices to preclude
  the dominant complement-coalition instability mechanisms."

Net effect: abstract from 240 → 242 words (still well within EJOR's
50-250 limit). The narrative form preserves every claim — no false
positives across multiple policies, asymptotic theory matches at
moderate sizes — while leaving exact ledger figures to the body's
results section (Table 4 onwards). The Operationally sentence
makes the practice-of-decision-making contribution visible in the
abstract, in line with EJOR's editorial preference.

No body content modified. No theorem statement, proof, table, figure,
or numerical result modified. Page count: main 30 / supp 13. URL still
`https://github.com/sage-Tae/online-tsg-2026`. Highlights file from
v2.5.5 unchanged.

## v2_5_5 (2026-05-06) — Comprehensive EJOR technical-screening compliance

After re-reading EJOR's Guide for Authors end-to-end, three remaining
non-compliances in v2.5.4 were identified and fixed in v2.5.5. No theorem,
proof, table, figure, or numerical result modified.

**Issue 1 — Abstract word count.** EJOR cap is 50-250 words. v2.5.4's
abstract was 294 words (44 over). Tightened to 240 words by:
- Splitting the 85-word "main theoretical result" sentence into two
  shorter sentences and dropping the redundant in-body cross-reference
- Removing the final 37-word "synthetic single-vehicle experiments"
  sentence whose content overlaps with the structural-blocking statement
  earlier in the abstract
- Tightening sentences #5 and #8 by 5-7 words each

**Issue 2 — Body acronym definitions.** EJOR forbids unqualified acronyms
in the main text. First-occurrence definitions added:
- Introduction (paragraph 4): "the optimal TSP tour on" →
  "the optimal traveling salesman problem (TSP) tour on"
- Introduction Contribution C1: "sequential LP cascade" →
  "sequential linear program (LP) cascade"

Subsequent uses of TSP and LP are unchanged because both are now formally
defined at first occurrence. TSG, VRG, BHH, NN, CI, BR, VRPTW were already
properly defined.

**Issue 3 — Highlights file.** EJOR requires a Highlights file
"mandatory from the First Submission" — 3-5 bullets, ≤85 chars each, no
abbreviations. v2.5.0 deferred this; v2.5.5 ships
`paper/highlights.txt` with five bullets covering the cooperative-game
model, the empty-Core mechanism characterization, the structural blocking
condition, and the empirical certification result.

Page count: main 30 / supp 13. URL still
`https://github.com/sage-Tae/online-tsg-2026`. TS-track and old-EJOR
files byte-identical.

## v2_5_4 (2026-05-06) — EJOR technical-screening compliance for the abstract

EJOR Guide for Authors prohibits formulae and unqualified acronyms in the
abstract. v2.5.3's abstract contained three math formulae ($k<n-1$ twice,
$\sqrt{n}$, $n$ grows) and one undefined acronym (LP). All replaced with
plain English in v2.5.4:

- "size-(n-1) and (n-2) coalitions" → "coalitions of size one less and two
  less than the grand coalition"
- "peak waiting queue satisfies $k<n-1$" → "peak waiting queue stays
  strictly below the grand-coalition size minus one"
- "Observation~\ref{obs:intermediate}" → "(characterized in the body)"
  (cross-references in abstracts also discouraged)
- "sequential LP cascade" → "sequential cascade of linear programs"
- "Beardwood–Halton–Hammersley $\sqrt{n}$ rate as $n$ grows" →
  "Beardwood–Halton–Hammersley square-root rate as the number of customers
  grows"
- "(peak queue $k<n-1$)" → "(a peak queue strictly below the grand-coalition
  size minus one)"

Body of the manuscript (Sections 1 onward) is unchanged. No theorem
statement, proof logic, table data, figure data, or numerical result
modified. Page count stays at 30; supplementary stays at 13. URL in
data-availability section still points to
`https://github.com/sage-Tae/online-tsg-2026`. TS-track and old-EJOR
files untouched.

## v2_5_3 (2026-05-02) — GitHub repository URL updated to `online-tsg-2026`

The v2.5.2 data-availability statement and cover letter pointed to
`https://github.com/sage-Tae/online-tsg`, the predecessor repository (v2.0
through v2.4 history). A clean replacement repository
`https://github.com/sage-Tae/online-tsg-2026` was created with EJOR-only
contents (no TS-track files, no pre-Prop-14 baseline) and the v2.5.2
release tagged. v2.5.3 syncs the URL change back into this submission tree:

- `paper/main_ejor.tex` line 762: `\url{...sage-Tae/online-tsg}` → `\url{...sage-Tae/online-tsg-2026}`
- `paper/cover_letter_ejor.md` line 95: `<...sage-Tae/online-tsg>` → `<...sage-Tae/online-tsg-2026>`

PDFs rebuilt: `main_ejor.pdf` 30 pp, `supplementary_ejor.pdf` 13 pp,
`cover_letter_ejor.pdf` 2 pp. URL `github.com/sage-Tae/online-tsg-2026`
verified embedded in both `main_ejor.pdf` and `cover_letter_ejor.pdf`.
0 broken refs.

No content change beyond the URL. TS-tree files (`main_trsc.{tex,pdf}`,
etc.) byte-identical.
